...
class Animacion(FuncAnimation):
    IMSHOW_KWARGS = {"interpolation": "quadric", "cmap": "seismic"}
    ...
    def __init__(self, dd):
        ...
        self.ln = plt.matshow(
            self.data[0],
            fignum=self.fignum,
            extent=(0, len(dd[0][:, 0]), 0, len(dd[0][0, :])),
            **self.IMSHOW_KWARGS)
        self.cb = plt.colorbar(ax=self.ax, orientation='horizontal')
        minv = 0e0
        maxv = 1e0
        for subdat in dd:
            minv = min(minv, subdat.min())
            maxv = max(maxv, subdat.max())
            pass
        self.cb.set_ticks(np.linspace(minv, maxv, 6))
        self.cb.set_ticklabels(np.linspace(minv, maxv, 6))
    ...
    def update(self, kk):
        self.ln = plt.matshow(
            self.data[kk],
            0,
            extent=(0, len(self.data[0][:, 0]), 0, len(self.data[0][0, :])),
            **self.IMSHOW_KWARGS)
...
print("Total nodes: {}".format(NX*NY))
print("\n Acquiring data... \n")
...
while M > 1:
    ...
    for i in range(0, M2):
        ...
        for j in range(0, NX):
            for k in range(0, NY):
                data[i, j, k] = float(f[10+i*T+j*L*NY+k*L:10+i*T+j*L*NY+(k+1)*L])
                pass
            pass
